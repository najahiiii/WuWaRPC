...
if not exists(config_path):
    fatal_error(
        console,
        indent(
            f"Config file does not exist. It should be located at {config_path}"
        ),
    )
...
def remove_startup_task(console: Console):
    ...
    """
    try:
        with console.status(indent("Removing the startup task..."),
                            spinner="dots"):
            delete_task_command = [
                "schtasks",
                "/delete",
            ...
            for file in files:
                if file.endswith(".lnk"):
                    ...
                    try:
                        ...
                        console.print(
                            indent(
                                f"Error reading shortcut {shortcut_path}: {e}"),
                            style="yellow",
                        )
...
def remove_shortcuts(console: Console, exe_path: str):
    ...
    for shortcut in shortcuts:
        try:
            ...
            console.print(indent(f"Failed to remove shortcut {shortcut}: {e}"),
                          style="red")
...
def delete_program_folder(console: Console):
    ...
    try:
        for root, _, files in os.walk(abspath(dirname(sys.executable))):
            for file in files:
                if normcase(normpath(join(root, file))) == normcase(
                    ...
                    continue
                with console.status(indent(f"Removing {file}..."),
                                    spinner="dots"):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".bat",
                                         mode="w") as temp_script:
        ...
        fatal_error(
            console,
            indent("Failed to remove the program folder",),
            e,
        )
...
console.input(
    indent("Press Enter to uninstall the Wuthering Waves Rich Presence..."))
...
try:
    ...
    exe_path = join(abspath(dirname(sys.executable)),
    ...
    print_divider(console, "[green]Removing the program folder[/green]",
                  "green")
    ...
    fatal_error(
        console,
        indent("An error occurred during uninstallation",),
        e,
    )
